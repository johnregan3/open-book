...
class Downloader:
    ...
    def download_passage(self, book, chapter, passage, file_path=""):
        ...
        return self.download_passage_range(
            book, chapter, passage, chapter, passage, file_path
        )
    ...
    def download_passages(self, book, chapter, passage_from, passage_to, file_path=""):
        ...
        """
        return self.download_passage_range(
            book, chapter, passage_from, chapter, passage_to, file_path
        )
    ...
    def download_chapter(self, book, chapter, file_path=""):
        ...
        """
        return self.download_passage_range(
            book, chapter, 1, chapter, common.get_end_of_chapter(), file_path
        )
    ...
    def download_chapters(self, book, chapter_from, chapter_to, file_path=""):
        ...
        """
        return self.download_passage_range(
            book, chapter_from, 1, chapter_to, common.get_end_of_chapter(), file_path
        )
    ...
    def download_book(self, book, file_path=""):
        ...
        return self.download_passage_range(
            ...
            1,
            1,
            common.get_chapter_count(book, self.translation),
            common.get_end_of_chapter(),
            file_path,
        )

    def download_passage_range(
        self, book, chapter_from, passage_from, chapter_to, passage_to, file_path=""
    ):
        """
        ...
        capped_chapter_from = common.get_capped_integer(
            chapter_from, max_value=common.get_chapter_count(book_name, translation)
        )
        capped_passage_from = common.get_capped_integer(passage_from)
        capped_chapter_to = common.get_capped_integer(
            chapter_to, max_value=common.get_chapter_count(book_name, translation)
        )
        ...
        chapter_range = range(1, common.get_chapter_count(book_name, self.translation) + 1)
        for chapter in chapter_range:
            ...
            if self.enable_multiprocessing:
                ...
                process = process_pool.apply_async(
                    self._get_passages_dict,
                    (online_bible, book_name, chapter, passage_initial, passage_final),
                ...
                document[book_name][self.__key_cast(chapter)] = self._get_passages_dict(
                    online_bible, book_name, chapter, passage_initial, passage_final
                )
        ...
        if self.enable_multiprocessing:
            ...
            document[book_name] = {
                self.__key_cast(chapter): process_results.pop(0).get()
                for chapter in chapter_range
            }

        if len(file_path) <= 0:
            file_location = os.path.join(
                self.default_directory, f"{book_name}{self.file_extension}"
            )
        else:
    ...
    def _get_passages_dict(self, online_bible, book, chapter, passage_from, passage_to):
        ...
        passage_list = online_bible.get_passages(
            book, chapter, passage_from, passage_to
        )
        ...
        is_translation_with_omitted_passages = (
            online_bible.translation in self.__translations_with_omitted_passages.keys()
        )
        passages = {}
        for passage in passage_list:
            if is_translation_with_omitted_passages:
                ...
                if (
                    passage_string
                    in self.__translations_with_omitted_passages[
                        online_bible.translation
                    ]
                ):
